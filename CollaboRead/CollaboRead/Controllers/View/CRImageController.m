...
- (void)loadView
{
	...
	self.sliceScroller = [[CRSliceScrollerViewController alloc] init];
	self.sliceScroller.delegate = self;
	NSArray *slices = ((CRScan*)self.caseChosen.scans[self.scanIndex]).slices;
	self.sliceScroller.slices = slices;
    ...
    [self.scansMenuController setViewFrame:CGRectMake(kToolPanelTableViewWidth, frame.size.height - self.sliceScroller.view.frame.size.height, 0, 0) animated:NO];
    self.scansMenuController.highlights = [[NSArray alloc] init];
    [self addChildViewController:self.scansMenuController];
    
    self.patientInfo = [[UITextView alloc] initWithFrame:CGRectMake(kToolPanelTableViewWidth, frame.size.height - self.sliceScroller.view.frame.size.height, 0, 0)];
    ...
    [view addSubview:self.sliceScroller.view];
    ...
    UIPanGestureRecognizer *scrollGesture = [[UIPanGestureRecognizer alloc] initWithTarget:self.sliceScroller action:@selector(scrollWithGesture:)];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    self.sliceScroller.view.frame = CGRectMake(self.imageMarkup.view.frame.origin.x, self.imageMarkup.view.frame.origin.y + self.imageMarkup.view.frame.size.height, self.imageMarkup.view.frame.size.width, kCR_CAROUSEL_CELL_HEIGHT + 20);
    self.sliceScroller.view.bounds = self.sliceScroller.view.frame;
}
...
- (void)setScanIndex:(NSUInteger)scanIndex
{
	...
	self.sliceScroller.slices = ((CRScan*)self.caseChosen.scans[scanIndex]).slices;
	[self.sliceScroller reloadData];
    self.sliceIndex = 0;
...
- (void)toggleScansMenu
{
    ...
    if (self.scansMenuController.view.frame.size.width == 0) {
        ...
        frame = CGRectMake(kToolPanelTableViewWidth, frame.size.height - size - self.sliceScroller.view.frame.size.height, size, size);
        [self.scansMenuController setViewFrame: frame animated:YES];
    }
    else {
        frame = CGRectMake(kToolPanelTableViewWidth, frame.size.height - self.sliceScroller.view.frame.size.height, 0, 0);
        [self.scansMenuController setViewFrame: frame animated:YES];
    }
}
...
- (void)togglePatientInfo
{
    ...
    if (self.patientInfo.frame.size.width == 0) {
        frame = CGRectMake(kToolPanelTableViewWidth, frame.size.height - (kPATIENT_INFO_DIMENSION) - self.sliceScroller.view.frame.size.height, kPATIENT_INFO_DIMENSION, kPATIENT_INFO_DIMENSION);
...
#pragma mark - CRSliceScroller Delegate Methods

- (void)sliceScroller:(CRSliceScrollerViewController *)sliceScroller didChangeSelectedIndex:(NSInteger)index
{
	[self.imageMarkup swapImageToScan:self.scanIndex slice:index];
	self.sliceScroller.view.frame = CGRectMake(self.imageMarkup.view.frame.origin.x, self.imageMarkup.view.frame.origin.y + self.imageMarkup.view.frame.size.height, self.imageMarkup.view.frame.size.width, kCR_CAROUSEL_CELL_HEIGHT + 20);
	self.sliceScroller.view.bounds = self.sliceScroller.view.frame;
